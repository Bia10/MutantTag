...
namespace Launcher
{
	internal static class Program
	{
		...
		public static void Build(bool launch)
		{
			...
			var csc = DEBUG ? "-debug -define:DEBUG" : string.Empty;
			...
			IEnumerable<string> coreSystemFiles = CSharpLua.CoreSystem.CoreSystemProvider.GetCoreSystemFiles()
				.Where(x => !x.EndsWith("Common.lua", StringComparison.Ordinal))
			...
			CompileResult compileResult = map.CompileScript(compiler, coreSystemFiles, blizzardJ, commonJ);
			...
			// Check if Warcraft III process is running
			Process[] processes = Process.GetProcessesByName("Warcraft III");
			if (processes.Length > 0)
			{
				// Terminate each instance of Warcraft III gracefully
				foreach (Process process in processes)
				{
					process.CloseMainWindow(); // Attempt to close the main window gracefully
					if (!process.WaitForExit(8000)) // Wait for up to 4 seconds for the process to exit
						process.Kill();
				}

				Console.WriteLine("Warcraft III process terminated.");
			}
			...
			if (File.Exists(wc3exe))
			{
				...
				var fileVersion = FileVersionInfo.GetVersionInfo(wc3exe).FileVersion;
				var isReforged = fileVersion is not null && Version.Parse(fileVersion) >= new Version(1, 32);
				if (isReforged)
